...
INSERT INTO user (name, age, address) VALUE ('김민정', 20, '서울특별시 마포구');
...
-- WHERE절: 특정 조건을 만족하는 절(record, tuple) 선택
SELECT * FROM user WHERE age >= 25; -- 나이가 25 이상인 사람
SELECT * FROM user WHERE id = 7; -- id가 7인 사람
SELECT name FROM user WHERE id = 7; -- id가 7인 사람의 이름만 조희
...
SELECT * FROM user WHERE id >= 6 ORDER BY age ASC; -- id 6 이상인 사람 중에서 age 컬럼 오름 차순
...
SELECT * FROM user ORDER BY name ASC; -- name 컬럼 오름차순 정렬 ''
...
-- 논리 연산자: AND, OR, NOT
SELECT * FROM user WHERE address IS NOT NULL AND age < 25; -- 둘중 하나만 참이어도 잡힘
SELECT * FROM user WHERE address IS NOT NULL OR age < 25; -- 둘 다 참이어야 함
SELECT * FROM user WHERE name LIKE '이%' AND age = 23;

-- [UPDATE - UPDATE]
-- 참고) Error Code : 1175
-- 테이블 데이터를 updqte/delete 하려고 할 때 key 컬럼을 이용해서 수정/삭제하는 것이 바람직
UPDATE user SET address = '서울특별시 강북구' WHERE id = 1;
UPDATE user SET address = '제주특별자치도 제주시', name = '이지현' WHERE id = 2;

-- 주의) update **where**절 미사용시, 모든 행의 데이터가 변경됨
-- update에서는 where절이 항상 따라다님 

-- [CREAT - DELETE]
-- : 데이터 삭제
-- 주의) delete where절 미사용시, 모든 형의 데이터가 삭제됨
...
-- 데이터 베이스에 접근해 읽거나 쓰는 것을 제한할 수 있는 권한 부여/박탈

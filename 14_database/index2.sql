...
-- 고객(customer) 데이터 추가
INSERT INTO customer (id, name, birthday) VALUE ('aaa', '김이현', '1990-03-17');
INSERT INTO customer (id, name, birthday) VALUE ('bbb', '최지율', '1992-11-01');
INSERT INTO customer (id, name, birthday) VALUE ('ccc', '이혜진', '1993-05-31');
...
INSERT INTO orderlist (customer_id, product_name, quantity) VALUE ('aaa', '맥북m1', 1);
INSERT INTO orderlist (customer_id, product_name, quantity) VALUE ('bbb', '빅파이', 31);
INSERT INTO orderlist (customer_id, product_name, quantity) VALUE ('ccc', '키보드', 3);
INSERT INTO orderlist (customer_id, product_name, quantity) VALUE ('bbb', '초코파이', 5);
INSERT INTO orderlist (customer_id, product_name, quantity) VALUE ('ccc', '귀여운텀블러', 1);
...
-- 유저아이디를 기준으로 customer의 orderlist 조인 (모든 컬럼에 대해 모든 행 보기)
SELECT *
FROM customer
	INNER JOIN orderlist
    ON customer.id = orderlist.customer_id;

-- 유저아이디를 기준으로 customer의 orderlist 조인 (모든 컬럼에 대해 WHERE절 만족하는 행 보기)
SELECT *
FROM customer
	INNER JOIN orderlist
    ON customer.id = orderlist.customer_id
...
-- 유저아이디를 기준으로 customer의 orderlist 조인 (일부 컬럼에 대해 모든 행 보기)

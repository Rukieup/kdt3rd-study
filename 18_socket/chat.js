...
// 유저 목록 업데이트 (유저 입장, 퇴장)
function updateList() {
  io.emit('updateNick', nickArray) // { socket.id:nick1, socket.id:nick2 }
};
...
io.on('connection', (socket) => {
  ...
  socket.on('setNick', (nick) => {
    // nick: 프론트에서 입력한 닉네임 값
    console.log('socket on setNick >> ', nick);

    // nickArray: {socket.Id1: nick1, socket.Id2:nice2, ...}
    // -> {nick1, nick2, nick3, ...}
    // -> nick이 존재하는지

    if ( Object.values(nickArray).indexOf(nick) > -1) {
      // 닉네임 중복이 있다면
      socket.emit('error', '아이디가 중복되었습니다.')
    } else {
      // 닉네임 중복이 없다면
...
// [실습44-3] 접속자 퇴장시
  ...
  socket.on('send', (data) => {
    ...
    if ( data.dm != 'all' ) {
      ...
      io.to(dmSocketId).emit('newMessage', sendData); // 특정 소켓아이디에게만 dm메시지 전송
      socket.emit('newMessage', sendData); // 
    } else {
      // data.dm: 특정 유저의 socekt id
